...
- 自动读取通达信本地沪深300、创业板指、科创50和北证50日线；缺少对应指数时保留绝对收益并明确降级。
- 统计同时给出绝对收益、基准收益和复合口径超额收益，并显示各周期完整率、中位滞后及独立时段样本数。
- 采用固定种子的 200 轮同板块、同期、相近波动随机窗口作为零假设基线，结果可复现。
- 新增默认权重 10 的幅度相似度；设为 0 可关闭。
- 行情缓存最多 7000 只且不超过浏览器配额的 20%，按最久未使用批量淘汰；配额不可读时只执行数量上限。
- 权息数据经过结构与复权连续性抽样自检，失败时整批退回原始价格并显示原因。
- 结果页展示证券、窗口、粗筛、全局、精排、DTW、去重和最终展示的完整漏斗。
