...
function selectCacheEvictions(records,{ver,maxCount,maxBytes=Infinity,targetRatio=.9}){
  ...
  const ordered=[...stale,...live],removed=[];
  ...
  for(const r of ordered){
    ...
    if(!mustRemove)break;
    removed.push(r.key);count--;bytes-=r.bytes??estimateRecordBytes(r);
  }
  return removed;
}
...
function samplePlaceboStarts(key,starts,limit=8,seed=20260703){
  const a=[...starts],rnd=seededRandom(stableHash(key+":"+seed)),k=Math.min(limit,a.length);
  for(let i=0;i<k;i++){const j=i+Math.floor(rnd()*(a.length-i));[a[i],a[j]]=[a[j],a[i]]}
  return a.slice(0,k);
}
function cacheWriteBackKeys(records,touched,evicted){return records.filter(r=>touched.has(r.key)&&!evicted.has(r.key)).map(r=>r.key)}
...
async function runMatch(cfg){
  ...
  const requiredBenchmarkKeys=[...new Set(keys.map(benchmarkKeyFor).filter(Boolean))],loadedBenchmarkKeys=[...benchmarkAgg.keys()],missingBenchmarkKeys=requiredBenchmarkKeys.filter(k=>!benchmarkAgg.has(k));
  ...
  const placeboPool=[];
  const addPlacebos=(key,stk,starts)=>{const board=boardOfKey(key),bench=benchmarkFor(key);for(const s of samplePlaceboStarts(key,starts,8)){const e=s+L-1;if(s<0||e>=stk.dates.length)continue;const fwd=forwardReturns(stk,e,bench),stats=windowStats(stk.closes,s,e);placeboPool.push({id:`${key}:${s}`,key,board,startD:stk.dates[s],endD:stk.dates[e],sd:stats.sd,fut:fwd.fut,benchmark:fwd.benchmark,excess:fwd.excess,lagBars:fwd.lagBars})}};
  ...
  post({type:"result",rows,statRows,meta:{mode,timeframe,recentBars:effectiveRecentBars,scanned:total,skipped,skipReasons,skipDetails,funnel:normalizeFunnel(funnel),placebo,benchmark:{loadedBenchmarkKeys,missingBenchmarkKeys},settings:{preset:cfg.preset||"custom",coarseThreshold,coarseThresholdEffective,K_COARSE,K_DTW,dtwBand},statSummary,L,refStartD,refEndD,elapsed:((Date.now()-t0)/1000).toFixed(1),
...
function forwardReturns(stk,e,benchmarkSeries=null){
  const n=stk.dates.length,fut={},benchmark={},excess={},lagBars={};
  for(const h of[5,10,20,60]){const hk="r"+h,valid=e+h<n;fut[hk]=valid?stk.closes[e+h]/stk.closes[e]-1:null;benchmark[hk]=valid?benchmarkReturn(benchmarkSeries,stk.dates[e],stk.dates[e+h]):null;excess[hk]=excessReturn(fut[hk],benchmark[hk]);lagBars[hk]=valid?n-1-e:null}
  return {fut,benchmark,excess,lagBars};
}
function packStk(stk,s,e,benchmarkSeries=null){
  ...
  const n=stk.dates.length;
  const {fut,benchmark,excess,lagBars}=forwardReturns(stk,e,benchmarkSeries);
  let mu=null,md=null;
  if(e+60<n){
